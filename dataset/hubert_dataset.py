...
from utils.common_utils import read_from_archive
import logging
...
class BucketizeBatchSampler(BatchSampler):
    ...
    def __init__(
        ...
        _LG.info(f"BucketizedBatchSampler length : {len(self.iter_list), self.batch_size}")
...
class DistributedBatchSampler(DistributedSampler):
    ...
    def __init__(
        ...
        _LG.info(f"DistributedBucketizedBatchSampler rank {self.rank} : {self.num_samples}")
...
class HuBERTDataSet(Dataset):
    ...
    def __init__(
        ...
        indexes_dir = self.exp_dir / "indexes"
        feat_archive_descriptors = {}
        labels_archive_descriptors = {}

        self.f_list, self.wav_archive, self.wav_bo, self.wav_bs, self.len_list, self.ind_list = self._get_lists(indexes_dir, dataset, subset)
        _LG.info(f"Finished loading wavs {subset} nb examples : {len(self.f_list), len(self.ind_list), max(self.ind_list)}")
        self.f_label, self.label_archive, self.label_bo, self.label_bs = self._load_labels(indexes_dir, dataset, subset)
        _LG.info(f"Total size of dataset {subset} :  {sum(self.len_list)/16000/3600} hours")
    ...
    def _get_lists(
        ...
        """
        _LG.info(f"Loading Wavs {subset}")
        wav_df = pd.read_csv(wav_dir / (subset + "_wav_index.csv"))

        _LG.info(f"Generating descriptors {subset}")

        self.wav_archive_descriptors = {f:open(f, "rb") for f in wav_df["archive"].unique()}
        return wav_df["path"].to_numpy(), wav_df["archive"].to_numpy(), wav_df["byte_offset"].to_numpy(), wav_df["byte_size"].to_numpy(), wav_df["num_frames"].to_numpy(), np.asarray(list(range(len(wav_df["path"]))))
    ...
    def _load_labels(self, label_dir: Path, dataset: str, subset: str) -> np.array:
        ...
        label_df = pd.read_csv(label_dir / (subset + "_label_index.csv"))
        _LG.info(f"Generating descriptors {subset}")
        self.label_archive_descriptors = {f:open(f, "rb") for f in label_df["archive"].unique()}
        return label_df["path"].to_numpy(), label_df["archive"].to_numpy(), label_df["byte_offset"].to_numpy(), label_df["byte_size"].to_numpy()
