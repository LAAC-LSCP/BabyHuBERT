...
def params_norm(params: torch.Tensor | Iterable[torch.Tensor], norm_type: float = 2.0, *, error_if_nonfinite: bool = False) -> torch.Tensor:
...
class HuBERTPreTrainModule(LightningModule):
    ...
    def training_step(self, batch: Batch, batch_idx):
        ...
        p_norm = params_norm(self.model.parameters())
        self.log("norm_weights", p_norm, on_step=True, on_epoch=True)
...
class HuBERTFineTuneModule(LightningModule):
    ...
    def training_step(self, batch: Batch_FineTune, batch_idx):
        ...
        self.log("Gathered_batch_size", batch_sizes.sum(), on_step=True, on_epoch=True)
